...
import type { Song, Instrument, Entry, LocalPartRef } from '../../../types';
...
export default function SectionEntryForm({
  ...
  const [entryParts, setEntryParts] = useState<LocalPartRef[]>([]);
  ...
  const toggleEntryPart = (instrumentId: string, partIndex: number) => {
    ...
    const exists = entryParts.some(p => isSameLocalPart(p, target));
    if (exists) {
      setEntryParts(prev => prev.filter(p => !isSameLocalPart(p, target)));
    } else {
      setEntryParts(prev => [...prev, target]);
    }
  };
          ...
          {entrySongId ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '0.75rem', maxHeight: '280px', overflowY: 'auto', border: '1px solid var(--border-color)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
              {availableParts.map(p => {
                const isChecked = entryParts.some(ep => isSameLocalPart(ep, p));
                return (
                  <div
